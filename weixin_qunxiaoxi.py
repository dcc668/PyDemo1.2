...
def get_name(id):
    ...
    if user!=None and user['RemarkName'] != '':
...
def update_function():
    ...
    def group_reply(msg):
        ...
        if from_userid != account1_id:
            ...
            if isat or room_name in keyroomnames:
...
if __name__ == '__main__':
    ...
    if os.path.exists(os.getcwd() + '/recent_name_list.pkl'):
        ...
        print('微信代理开始,当前聊天对象%s' % (recent_name_list[chat_namelist_index]));
    group_ids=[]
    for rooms in itchat.get_chatrooms():
        group_id=rooms['UserName']
        group_ids.append(group_id)
    pool=Pool()
    pool.map(get_room_chat_history,group_ids)
    ...
    while 1:
        ...
        if localtime.tm_min in [0, 30] and localtime.tm_hour >= 7 and localtime.tm_sec in [0, 1]:
            ...
            print('微信代理正常(%d:%d),当前聊天对象%s' % (
            localtime.tm_hour, localtime.tm_min, recent_name_list[chat_namelist_index]))
            time.sleep(1)
