...
class ETL:
	"""Extract Transform Load class for all data operations pre model inputs.
		Data is read in generative way to allow for large datafiles
	...
	def create_clean_datafile(self, filename_in, filename_out, batch_size=1000, x_window_size=100, y_window_size=1, y_col=0, filter_cols=None, normalise=True):
		...
		with h5py.File(filename_out, 'w') as hf:
			x1, y1 = next(data_gen)#x_window_size 行 x数据,y_window_size 行 y数据,
			#Initialise hdf5 x, y datasets with first chunk of data
			rcount_x = x1.shape[0]#三维数组,（个数，行，列）
			...
			for x_batch, y_batch in data_gen:
				...
				print('> 批处理处理原始文件,返回 x & y  | 返回次数:', i, end='\r')
		...
		print('创建H5文件：' + filename_out + '.h5`')

	#数据预处理
	#filter_cols:需要保留的列（数组）
	#y_col :输出列,y-predict column name
	#normalise： 是否归一化处理
	def clean_data(self, filepath, batch_size, x_window_size, y_window_size, y_col, filter_cols, normalise):
		...
		while((i+x_window_size+y_window_size) <= num_rows):
			x_window_data = data[i:(i+x_window_size)]#取x_window_size行
			y_window_data = data[(i+x_window_size):(i+x_window_size+y_window_size)]#取y_window_size行
			...
			if(normalise):#是否归一化处理
				abs_base, x_window_data = self.zero_base_standardise(x_window_data)#abs_base：x_window_data的第一行
				_, y_window_data = self.zero_base_standardise(y_window_data, abs_base=abs_base)

			#Average of the desired predicter y column
			y_average = np.average(y_window_data.values[:, y_col])#输出：取每列的平均值
	...
	def zero_base_standardise(self, data, abs_base=pd.DataFrame()):
		...
		data_standardised = (data/abs_base)-1#对应位置上相除
