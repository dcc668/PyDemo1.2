...
def predict(cropModel,classModel):
    ...
    #从一个生成器上获取数据并进行预测
    predictCrop = cropModel.predict_generator(cropGenerator,max_queue_size=121)
    maping = [] # sorting images
    for imName in cropFilenames:
        maping.append(int(re.sub('\D*','',imName)))
    goodArr = predictCrop[:,1]#取第2列(相当于： train/1 目录下文件)
    print('predictCrop: {}'.format(goodArr))
    index2 = np.argsort(goodArr).tolist()#argsort函数返回的是数组值从小到大的索引值
    index = []
    for p in index2:
        index.append(maping[p])
    ...
    classGenerator = datagenClass.flow_from_directory(
        ...
        shuffle = False,
        target_size=(60,30))
    predictClass = classModel.predict_generator(classGenerator,4)#4:Total number of steps (batches of samples)
    ...
    classIndex = []
    temp = [0,0,0,0]
    result = predictClass.argmax(axis = 1)#第axis维最大数的索引
    result.tolist()
...
if __name__ == '__main__':
    ...
    timeStamp = int(time.time()*1000)
    captchaUrl = url + '/captcha.gif?=' + str(timeStamp)
    f = open('temp/captcha.gif','wb')
    s = requests.session()
    f.write(s.get(captchaUrl, headers=headers).content)
    f.close()
    im = Image.open('temp/captcha.gif')
    im.save('temp/captcha.png')
    im = Image.open('temp/captcha.png')
    crop(im)
    res= predict(cropModel,classModel)
