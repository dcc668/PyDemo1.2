...
class BaseDao:
    ...
    def updateUserNoLock(self, user):
        ...
        try:
            ...
            sql = "select status from users where id = %d;" \
                  ...
                  ",userName = '%s'" \
                  ",password = '%s'" \
                  ",email = '%s',money=%d,status=%d where id=%d and status=%d;" \
                  % (user.id, user.status, user.userName, user.password, user.email, user.money,user.status+1,user.id,user.status)
    ...
    def updateUser(self, user):
        ...
        try:
            self.conn.autocommit(False)
            cursor =  self.conn.cursor(cursor=cursors.DictCursor)
            sql1 = "start transaction; select status from users where id = %d for update;" \
            "update users set status = %d" \
                      ",userName = '%s'" \
                      ",password = '%s'" \
                      ",email = '%s',money=%d where id=%d;" \
                      % (user.id,user.status, user.userName, user.password, user.email, user.money,user.id)
            print('....execute...sql.....'+sql1)
            cursor.execute(sql1)
            time.sleep(5)
            self.conn.commit()
        except Exception as e:
    ...
    def run(self):
        ...
        if (self.sym):
            ...
            try:
                ...
                msg='更新失败发生冲突'
        self._signal.emit(str(self.times),msg);
...
class Ui_MainWindow(object):
    ...
    def buy_goods(self):
        ...
        t.start()
    def callbacklog(self,times,msg):
        self.modifyCiShu()
        print('--->>>'+msg)
        if msg=='':
            con='第'+str(CISHU)+'次消费结束，耗时：'+str(times)
        else:
            con = '第' + str(CISHU) + '次消费结束,'+msg;
        self.modifyMEMO(con)
        self.textB.setText(MEMO)
        print(MEMO)
        self.scrollArea.setWidget(self.textB)
