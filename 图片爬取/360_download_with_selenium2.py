...
class DownLoad():
    ...
    def bing_gen_query_url(keywords):
        base_url = "http://image.so.com/i?q=%s&src=srp" % (keywords)
        return base_url
    ...
    def bing_image_url_from_webpage(driver):
        ...
        retry_count = 0
        while True:
            image_elements = driver.find_elements_by_xpath("//a[@class='img_box img_zoom']")
            if len(image_elements) > img_count:
                img_count = len(image_elements)
                driver.execute_script(
                    "window.scrollTo(0, document.body.scrollHeight);")
            else:
                smb = driver.find_elements_by_class_name("btn_loadmore")
        ...
        for image_element in image_elements:
            ...
            comicUrl = image_url.strip("http://")
            comicUrl = "http://" + comicUrl
            print('------------------>>>>>image_url:' + str(comicUrl))
            driver.get(comicUrl)
            download_url=driver.find_element_by_id('lbDownload')
            link=download_url.get_attribute('qh_download')
            print('------------------>>>>>download_url:' + str(link))
            print('------------------>>>>>download_url html:' + str(driver.page_source))
            image_urls.append(link)
            # regex = r'qh_download="(.*?)"'
            # download_urls = re.findall(regex, str(driver.page_source), re.S)
            # if len(download_urls) > 0:
            #     print('------------------>>>>>download_urls:' + str(download_urls[0]))
            #     image_urls.append(download_urls[0])
                # except Exception as e:
                #     print('获取详细链接发生异常：'+str(e))
                #     time.sleep(1)
        return image_urls
...
class Google():
    ...
    def get_image_links(self, main_keywords, pre_link_path, fileNames):
...
if __name__ == "__main__":
    ...
    download_dir = 'D:/pic4-10/douguo/'
    link_files_dir = 'D:/pic4-10/douguo/url/'
    log_dir = 'D:/pic4-10/douguo/logs/'
