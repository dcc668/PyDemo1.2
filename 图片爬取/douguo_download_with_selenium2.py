...
class DownLoad():
    ...
    def bing_gen_query_url(keywords, page):
        base_url = "http://www.douguo.com/search/recipe/%s/%d"%(keywords,page)
        return base_url
        ...
        while True:
            ...
            if len(image_elements) > img_count:
                ...
                if retry_count>=2:#重试
                    ...
                    print('重试.....')
                    continue
            time.sleep(1)#default 3
        for image_element in image_elements:
            print('------------------>>>>>image_element:' + str(image_element))
            image_urls.append(image_element[0])
        return image_urls
...
class Google():
    ...
    def get_image_links(self, main_keywords, pre_link_path, fileNames,page=2):
        ...
        for i in range(len(main_keywords)):
            ...
            for j in range(page):
                url = DownLoad.bing_gen_query_url(search_query,j*20)
                self.driver.get(url)
